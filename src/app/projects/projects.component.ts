...
export class ProjectsComponent implements OnInit {
  ...
  filterPublications() {
  	this.practicesPublications = this.allPublications.filter((pub:{}) => {
  		return ['chi25c', 'cscw24a', 'chi24d', 'chi23a', 'cscw22b', 'chi22a', 'cscw21a','jmh21','dis21b','chi21a', 'chi21b', 'cscw20b', 'chi17a', 'chi16c', 'ubi15', 'chi15'].includes(pub['key']);
  	});

  	this.designPublications = this.allPublications.filter((pub:{}) => {
  		return ['chi25a', 'chi24a', 'imwut24', 'mobilehci23', 'imwut22', 'imwut21','dis21a', 'ubi16', 'chi16a', 'dis14', 'pervasivehealth20'].includes(pub['key']);
  	});

  	this.socialPublications = this.allPublications.filter((pub:{}) => {
  		return ['cscw25', 'chi25d', 'cscw24c', 'chi23c', 'cscw22d', 'cscw21c', 'cscw20a', 'gi20', 'cscw17_online', 'chi16b', 'cscw15', 'ubi13'].includes(pub['key']);
  	});

    this.clinicalPublications = this.allPublications.filter((pub:{}) => {
      return ['chi25b', 'cscw24b', 'chi24c', 'jmh23', 'chi23b','chi22b', 'cscw22a', 'cscw21b', 'dis18', 'chi17b'].includes(pub['key']);
    });
